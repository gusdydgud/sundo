...
class BusinessViewModel(private val businessRepository: BusinessRepository) : ViewModel() {
    ...
    // 비즈니스 목록을 로드하는 함수 (로컬 및 서버)
    fun loadBusinessList() {
        viewModelScope.launch {
            try {
                val businesses = businessRepository.getBusinessList()
    ...
    fun addBusiness(business: Business) {
        viewModelScope.launch {
            try {
                ...
                _error.value = "비즈니스를 추가하는 중 오류가 발생했습니다: ${e.localizedMessage}"
            }
        }
    }
    ...
    fun updateBusiness(business: Business) {
        ...
        viewModelScope.launch {
            try {
                val updatedBusiness = businessRepository.updateBusiness(bno, business)
                val index = businessItems.indexOfFirst { it.bno == updatedBusiness.bno }
                if (index != -1) {
                    businessItems[index] = updatedBusiness
                    _businessList.value = businessItems.toMutableList()
                }
            } catch (e: Exception) {
                _error.value = "비즈니스를 수정하는 중 오류가 발생했습니다: ${e.localizedMessage}"
            }
        }
    }
